...
contrasts(main_trials_long_rm$inference_type)

rm_cs3_inter <- brm(
  rating ~ condition * inference_type + (1 + condition * inference_type | item_id) + (1 + condition * inference_type | submission_id),
...
compare_groups(
  rm_cs3_inter,
  higher = condition == "upper bound",
  lower = condition == "proposals"
)

compare_groups(
  rm_cs3_inter,
  higher = condition == "proposals",
  lower = condition == "lower bound"
)
```